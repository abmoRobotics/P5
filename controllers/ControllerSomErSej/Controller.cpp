...
void Controller::ForwardKinematics(float theta, float thetad)
{
  ...
  float T1 = sqrt(square(L0) + square(L1) - 2 * L0 * L1 * cos(theta));
  float V1 = acos((square(T1) + square(L1) - square(L0)) / (2 * T1 * L1));
  float V2 = deg2rad(180) - theta - V1;
  float thetadH = thetad - V2;
  float T2 = sqrt(square(L4) + square(T1) - 2 * L4 * T1 * cos(thetadH));
  float V12 = acos((square(T1) + square(T2) - square(L4)) / (2 * T1 * T2));
  float thetaV = V1 + V12;
  float thetaTA = acos((square(T2) + square(L2) - square(L3)) / (2 * T2 * L2));

  float thetaL = thetaTA + thetaV;
  float T3 = sqrt(square(L1) + square(L2) - 2 * L1 * L2 * cos(thetaL));
  float thetaTB = acos((square(T3) + square(L2) - square(L1)) / (2 * T3 * L2));
  float thetaTC = deg2rad(180) - thetaL - thetaTB;
  float thetaXY = theta - thetaTC;
